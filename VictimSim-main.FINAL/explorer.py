...
class Explorer(AbstractAgent):
    ...
    def Astar(self, position):
        ...
        while True:
            ...
            while len(disponiveis) != 0:
                ...
                if nextPos[0] != 0 and nextPos[1] != 0:
                    g = self.COST_DIAG
                    f = g + self.COST_BACK + self.calc_heuristic()
                else:
                    g = self.COST_LINE
                    f = g + self.COST_BACK + self.calc_heuristic()
            ...
            if count > 0:  # Tendo opções pra ir, vai pra melhor escolhida (mais proxima da base). Ps: percebi que dá pra usar o len(disponiveis).
                ...
                if a[0] != 0 and a[1] != 0:
                    self.COST_BACK -= self.COST_DIAG
                else:
                    self.COST_BACK -= self.COST_LINE
            # print(checked)
            # print("X:", x, "Y:", y)
        ...
        if self.COST_BACK < (self.rtime - (self.COST_DIAG if self.COST_DIAG > self.COST_LINE else self.COST_LINE) - self.COST_READ):  # Teste para ver se a bateria esta acabando, dado que ele precisa fazer um movimento de volta, precisa sobrar o suficiente para uma ação (explico melhor no whats se precisar)
            self.pathHome = []
